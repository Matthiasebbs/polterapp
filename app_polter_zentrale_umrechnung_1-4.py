...
def extract_fraechter_from_filename(filename):
    """Frächter aus dem Ende des PDF-Dateinamens ableiten."""
    stem = Path(filename).stem.strip()

    # Beispiele: ...-Soller, ...-Ammer, ...-G&H
    if "-" in stem:
        last = stem.rsplit("-", 1)[-1].strip()
        if last and not re.fullmatch(r"\d+(?:_\d+)*", last) and "_" not in last:
            return last

    # Beispiele: ... Astner, ... Hunglinger, ... Mösl
    parts = stem.split()
    if len(parts) >= 2:
        last = parts[-1].strip()
        if last and not re.fullmatch(r"\d+(?:_\d+)*", last):
            return last
...
with right:
    ...
    if not view.empty:
        ...
        with st.form("edit"):
            st.caption("Umrechnung: 1,4 RM = 1 FM. Ändere einfach RM oder FM – das jeweils andere Feld wird beim Speichern automatisch neu berechnet.")
...
def highlight_completed(row):
    """Erledigte Polter in der Tabelle grün hervorheben."""
    if str(row.get("Status", "")) == "Erledigt":
        return ["background-color: #d9ead3; color: #1f4e1f"] * len(row)
    return [""] * len(row)

styled_show = show.style.apply(highlight_completed, axis=1)
...
st.caption("Grün markierte Zeilen = erledigte Polter. Diese werden auf der Karte nicht mehr angezeigt.")
